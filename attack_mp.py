#!/usr/bin/env python3
from collections import deque
from multiprocessing import Pool
from sympy.solvers import solve
from sympy import Symbol

class WeinerAttack:

    expansions = []
    phis = []

    def __init__(self, e, N):
        self.e = e
        self.N = N

    def continued_fraction(self):
        num = self.e
        den = self.N
        while num != 1:
            expansion = num // den
            remainder = num % den
            num = den
            den = remainder
            self.expansions.append(expansion)

    def convergents(self):
        k = deque([0, 1], 2)
        h = deque([1, 0], 2)
        for i, expansion in enumerate(self.expansions[1:]):
            k_new = expansion * k[1] + k[0]
            h_new = expansion * h[1] + h[0]
            k.append(k_new)
            h.append(h_new)

            yield k_new, h_new

    def phis(self):
        self.phi = [(self.e * k - 1) // h for k, h in self.convergents()]

    def phi_chunks(self, n):
        return [self.phi[i:i+n] for i in range(0, len(self.phi), n)]

    def solve_poly(self, phis):
        x = Symbol('x')
        for phi in phis:
            p = [self.N - phi + 1, self.N]
            roots = solve(x**2 -  p[0]*x + p[1], x)

            if roots[0] * roots[1] == self.N:
                raise ValueFound(roots)

    def attack(self, num_process = 8):
        self.continued_fraction()
        self.phis()
        try:
            with Pool(num_process) as p:
                p.map(self.solve_poly, self.phi_chunks(num_process)) 
        except ValueFound as e:
            return e.value
        except Exception as e:
            print(e)

class ValueFound(Exception):
    def __init__(self, value):
        self.value = value

def main():
    e = 
    n = 
    attack = WeinerAttack(e, n)
    factor = attack.attack()
    print('p:', factor[0], '\nq:', factor[1])

if __name__ == '__main__':
    main()
